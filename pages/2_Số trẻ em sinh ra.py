...
# Chuyển giá trị về số thập phân với 2 chữ số sau dấu phẩy
    ...
    df_selected = df[df['indicator'].isin(['soTreEmSinhRa'])]
...
# Lọc dữ liệu để chỉ lấy thông tin về diện tích
    df_tresinhra = df_selected_year[df_selected_year['indicator'] == 'soTreEmSinhRa']
# Tạo biểu đồ cột cho dân số
    fig_trersinhra = px.bar(
    df_tresinhra,
    x='district',
    y='indicator_value',
    labels={'indicator_value': 'Số trẻ em sinh ra', 'district': 'Quận/Huyện','year' :'Năm'},
    hover_data={'year': True},
    category_orders={"district": df_tresinhra.sort_values("indicator_value")["district"]},  # Sắp xếp theo giá trị của indicator_value
)
    fig_trersinhra.update_xaxes(categoryorder='total descending', tickangle=45)
...
# Đặt nội dung vào cột 1
    ...
    col1.plotly_chart(fig_trersinhra)
...
# Tính giá trị tối đa của soTreEmSinhRa và danSoTrungBinh
    max_soTreEmSinhRa = json["features"].apply(lambda row: row["properties"]["soTreEmSinhRa"]).max()

# Tính giá trị của BREAKS tùy thuộc vào giá trị tối đa của soTreEmSinhRa
    BREAKS = [max_soTreEmSinhRa * i / 10 for i in range(1, 11)]
...
# Parse the geometry out in Pandas
    ...
    df_JSON["soTreEmSinhRa"] = json["features"].apply(lambda row: row["properties"]["soTreEmSinhRa"])
    df_JSON["name"] = json["features"].apply(lambda row: row["properties"]["name"])
    df_JSON["elevation_soTreEmSinhRa"] = json["features"].apply(lambda row: calculate_elevation(row["properties"]["soTreEmSinhRa"]))

# Tính toán giá trị màu sắc tương ứng cho soTreEmSinhRa
    def get_fill_color_soTreEmSinhRa(value):
     for i, b in enumerate(BREAKS):
        if value < b:
            return COLOR_RANGE[i]
     return COLOR_RANGE[-1]

    df_JSON["fill_color_soTreEmSinhRa"] = df_JSON["soTreEmSinhRa"].apply(get_fill_color_soTreEmSinhRa)
# Set up PyDeck for PolygonLayer with Tooltip
    polygon_layer = pdk.Layer(
    "PolygonLayer",
    df_JSON,
    id="geojson_soTreEmSinhRa",
    opacity=0.8,
    stroked=False,
    ...
    extruded=True,
    wireframe=True,
    get_elevation="elevation_soTreEmSinhRa",
    get_fill_color="fill_color_soTreEmSinhRa",
    get_line_color=[255, 255, 255],
    auto_highlight=True,
    pickable=True,
)
    tooltip = {"html": "<b>Số trẻ em sinh ra:</b> {soTreEmSinhRa} <br /><b>Quận/huyện:</b> {name}"}
