...
st.info("Tỷ suất sinh thô phân theo quận/huyện")
...
df_selected = df[df['indicator'].isin(['tySuatSinhTho'])]
...
df_tysuat = df_selected_year[df_selected_year['indicator'] == 'tySuatSinhTho']
# Tạo biểu đồ cột cho dân số
fig_tysuat = px.bar(
    df_tysuat,
    x='district',
    y='indicator_value',
    labels={'indicator_value': 'Tỷ suất sinh thô', 'district': 'Quận/Huyện','year' :'Năm'},
...
with col1:
    st.info("Biểu đồ Tỷ suất sinh thô theo Quận/Huyện")
    col1.plotly_chart(fig_tysuat)

# Đặt nội dung vào cột 2
with col2:
    st.info("Bản đồ Tỷ suất sinh thô theo Quận/Huyện")
...
# Tính giá trị tối đa của tySuatSinhTho và danSoTrungBinh
max_tySuatSinhTho = json["features"].apply(lambda row: row["properties"]["tySuatSinhTho"]).max()

# Tính giá trị của BREAKS tùy thuộc vào giá trị tối đa của tySuatSinhTho
BREAKS = [max_tySuatSinhTho * i / 10 for i in range(1, 11)]
...
df_JSON["tySuatSinhTho"] = json["features"].apply(lambda row: row["properties"]["tySuatSinhTho"])
df_JSON["name"] = json["features"].apply(lambda row: row["properties"]["name"])
df_JSON["elevation_tySuatSinhTho"] = json["features"].apply(lambda row: calculate_elevation(row["properties"]["tySuatSinhTho"]))

# Tính toán giá trị màu sắc tương ứng cho tySuatSinhTho
def get_fill_color_tySuatSinhTho(value):
    for i, b in enumerate(BREAKS):
        if value < b:
            return COLOR_RANGE[i]
    return COLOR_RANGE[-1]

df_JSON["fill_color_tySuatSinhTho"] = df_JSON["tySuatSinhTho"].apply(get_fill_color_tySuatSinhTho)

# Set up PyDeck for PolygonLayer with Tooltip
polygon_layer = pdk.Layer(
    "PolygonLayer",
    df_JSON,
    id="geojson_tySuatSinhTho",
    opacity=0.8,
    stroked=False,
    ...
    extruded=True,
    wireframe=True,
    get_elevation="elevation_tySuatSinhTho",
    get_fill_color="fill_color_tySuatSinhTho",
    get_line_color=[255, 255, 255],
    auto_highlight=True,
    pickable=True,
)
tooltip = {"html": "<b>Tỷ suất sinh thô:</b> {tySuatSinhTho} <br /><b>Quận/huyện:</b> {name}"}
